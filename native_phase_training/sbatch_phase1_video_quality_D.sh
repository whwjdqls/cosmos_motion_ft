#!/bin/bash
# Ablation D: variable causal prefix with camera-token-only generation K/V LoRA.
# I2V is evaluation-only because its packed camera-token mask is empty.
#SBATCH -p a3ultra
#SBATCH --nodes=1
#SBATCH --gres=gpu:8
#SBATCH --ntasks=1
...
#SBATCH --job-name=np1vqD
#SBATCH --output=/home/jungbin_cho/cosmos_motion_ft/slurm-np1vqD-%j.out
#SBATCH --exclusive
...
export NATIVEP1_RUN_NAME=native_phase1_vq_D_varprefix_camera_kv_lora_aw2_bs4_lr5e5_ema100k_qfilterv1_person
