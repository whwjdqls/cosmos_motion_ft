...
world_camera_nymeria_latent_nano = LazyDict(
    dict(
        ...
        model=L(LatentOmniMoTModel)(config=copy.deepcopy(_MODEL_CONFIG), _recursive_=False),
        optimizer=dict(
            betas=[0.9, 0.99],
            eps=1.0e-08,
            fused=True,
...
world_camera_nymeria_latent_nano["model"]["config"]["lora_enabled"] = not _FULL_FT
