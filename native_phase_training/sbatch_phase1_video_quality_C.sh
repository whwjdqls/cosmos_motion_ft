#!/bin/bash
# Ablation C: variable causal prefix with only the camera/action interface trainable.
# I2V is evaluation-only because it has no action tokens and no trainable path.
#SBATCH -p a3ultra
#SBATCH --nodes=1
#SBATCH --gres=gpu:8
#SBATCH --ntasks=1
...
#SBATCH --job-name=np1vqC
#SBATCH --output=/home/jungbin_cho/cosmos_motion_ft/slurm-np1vqC-%j.out
#SBATCH --exclusive
...
export NATIVEP1_ADAPTATION_MODE=action_only
export NATIVEP1_PREFIX_LENGTHS=1,9,17,33,49
export NATIVEP1_PREFIX_SAMPLING_WEIGHTS=
export NATIVEP1_RUN_NAME=native_phase1_vq_C_varprefix_action_only_aw2_bs4_lr5e5_ema100k_qfilterv1_person
