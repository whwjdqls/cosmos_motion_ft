"""Contact-aware physical losses for UniEgo motion training."""
...
def contact_aware_losses(
    ...
    *,
    fps: float = 20.0,
    contact_logit_scale: float = 2.0,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Return balanced contact BCE, planted-foot velocity, and foot-height losses.

    Contact BCE is evaluated in raw UniEgo contact space. Physical losses use
    ground-truth contact masks, preventing the model from avoiding them by
    predicting no contact. Horizontal velocity is measured in physical m/s;
    height reconstruction uses the equivalent raw local-pose Y channels to
    avoid unstable gradients through the cumulative trajectory decoder.
    """
    if fps <= 0.0:
