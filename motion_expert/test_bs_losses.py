"""Focused CPU tests for BONES contact-aware losses."""
...
from bs_losses import contact_aware_losses
...
class ContactAwareLossTest(unittest.TestCase):
    ...
    def losses(self, x0_hat, joints_hat):
        return contact_aware_losses(
            ...
            self.std,
            fps=20.0,
            contact_logit_scale=10.0,
        )

    def test_exact_contacts_and_stationary_feet_have_small_losses(self):
        contact, velocity, height = self.losses(self.x0.clone(), self.joints_gt.clone())
        self.assertLess(float(contact), 0.01)
    ...
    def test_wrong_contacts_increase_balanced_bce(self):
        ...
        wrong[..., FOOT_SLICE] = (wrong_raw - self.mean[FOOT_SLICE]) / self.std[FOOT_SLICE]
        correct_loss, _, _ = self.losses(self.x0.clone(), self.joints_gt.clone())
        wrong_loss, _, _ = self.losses(wrong, self.joints_gt.clone())
        self.assertGreater(float(wrong_loss), float(correct_loss) * 100.0)
